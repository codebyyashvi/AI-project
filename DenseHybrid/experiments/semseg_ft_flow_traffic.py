...
from torchvision.utils import save_image
from data import colorize_cityscapes_labels


class SemsegFlowNegativesTrafficExperiment(Experiment):

    def __init__(self, model, optimizer, loaders, epochs, logger, device, cp_file, args, flow, flow_optim, img_dir,
                 data_loader):
        super(SemsegFlowNegativesTrafficExperiment, self).__init__(model, optimizer, loaders, epochs, logger, device,
                                                                   cp_file, args)
    ...
    def train(self):
        ...
        with tqdm(total=len(self.train_loader.dataset)) as progress_bar:
            for batch_idx, data in enumerate(self.train_loader, 1):
                ...
                x, id_patch, label = self._paste_square_patch(x_.clone(), label_.clone(), ood_patch,
