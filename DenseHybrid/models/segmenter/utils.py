...
def inference_th(
    ...
    C = model.n_cls
    seg_map = torch.zeros((C, ori_shape[0], ori_shape[1]), device=device)
    seg_map_th = torch.zeros((2, ori_shape[0], ori_shape[1]), device=device)
    for im, im_metas in zip(ims, ims_metas):
        im = im.to(device)
        im = resize(im, window_size)
        flip = im_metas["flip"]
        windows = sliding_window(im, flip, window_size, window_stride)
        windows_th = sliding_window(im, flip, window_size, window_stride)
        crops = torch.stack(windows.pop("crop"))[:, 0]
        B = len(crops)
        WB = batch_size
        seg_maps = torch.zeros((B, C, window_size, window_size), device=im.device)
        seg_maps_th = torch.zeros((B, 2, window_size, window_size), device=im.device)
        with torch.no_grad():
            for i in range(0, B, WB):
                out = model.forward(crops[i: i + WB])
                seg_maps[i: i + WB] = out[0]
                seg_maps_th[i: i + WB] = out[1]
        windows["seg_maps"] = seg_maps
        windows_th["seg_maps"] = seg_maps_th
        im_seg_map = merge_windows(windows, window_size, ori_shape)
        im_seg_map_th = merge_windows(windows_th, window_size, ori_shape)

        seg_map += im_seg_map
        seg_map_th += im_seg_map_th
    seg_map /= len(ims)
    seg_map_th /= len(ims)

    return seg_map, seg_map_th
