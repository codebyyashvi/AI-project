        ...
        for i, t in enumerate(self.children()):
            ...
            if i == 1:
                if self.store_samples and self.step < 10:
                    N, C, H, W = x.shape
                    self.sample_buffer = torch.zeros(10 * N, C, H, W) if isinstance(self.sample_buffer,
                                                                                    type(None)) else self.sample_buffer
...
class LadderDenseNet(nn.Module):
    def __init__(self, args, num_classes=19, checkpointing=False):
        ...
        self.upsample = UpsampleWrapper(args, self.spp.num_features, self.backbone.skip_sizes,
                                        checkpointing=checkpointing)
        self.logits = _BNReluConv(self.upsample.num_features, self.num_classes, k=1, bias=True,
                                  checkpointing=checkpointing)
