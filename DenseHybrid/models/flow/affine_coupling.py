from models.flow.denseflow.transforms import Bijection, BatchNormBijection2d, Conv1x1
from models.flow.denseflow.utils import sum_except_batch
from models.flow.denseflow.nn.layers import ElementwiseParams2d
import torch.utils.checkpoint as cp
import torch.nn.functional as F
from models.flow.denseflow.nn.nets import DenseNet, DenseNetMultihead, PureDenseNet, ResNet
...
class Standard(nn.Module):
    def __init__(self, inChannels, midChannel, outChannel, checkpointing=False):
        ...
        self.transform = nn.Sequential(
            ...
            nn.Conv2d(inChannels, midChannel, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(midChannel),
            nn.ReLU(inplace=True),
            nn.Conv2d(midChannel, outChannel, kernel_size=3, padding=1, bias=False)
        )

        nn.init.zeros_(self.transform[-1].weight)

        self.checkpointing = checkpointing
        self.cp_func = _checkpoint_fb(self.transform)
...
class Bottleneck(nn.Module):
    def __init__(self, inChannels, midChannel, outChannel, checkpointing=False):
        ...
        self.transform = nn.Sequential(
            nn.BatchNorm2d(inChannels),
            nn.ReLU(inplace=True),
            nn.Conv2d(inChannels, midChannel, kernel_size=1, bias=True),
            nn.BatchNorm2d(midChannel),
            nn.ReLU(inplace=True),
            nn.Conv2d(midChannel, midChannel, kernel_size=3, padding=1, bias=True),
            nn.BatchNorm2d(midChannel),
            nn.ReLU(inplace=True),
            nn.Conv2d(midChannel, outChannel, kernel_size=1, bias=True)
        )
...
class DN(nn.Module):
    def __init__(self, inChannels, midChannel, outChannel, checkpointing=False, depth=7):
...
class RN(nn.Module):
    def __init__(self, inChannels, midChannel, outChannel, checkpointing=False):
        ...
        self.rn = ResNet(in_channels=inChannels,
                         out_channels=outChannel,
                         num_blocks=1,
                         mid_channels=midChannel,
                         dropout=0.0,
...
class Custom(nn.Module):
    def __init__(self, inChannels, midChannel, outChannel, checkpointing=False):
        ...
        self.transform = nn.Sequential(
            nn.BatchNorm2d(inChannels),
            nn.ReLU(inplace=True),
            nn.Conv2d(inChannels, midChannel, kernel_size=1, bias=False),
            nn.BatchNorm2d(midChannel),
            nn.ReLU(inplace=True),
            nn.Conv2d(midChannel, midChannel, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(midChannel),
            nn.ReLU(inplace=True),
            nn.Conv2d(midChannel, midChannel, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(midChannel),
            nn.ReLU(inplace=True),
            nn.Conv2d(midChannel, outChannel, kernel_size=1, bias=False)
        )
...
class SingleAffineCoupling(AdvancedAffineCouplingBijection):
    ...
    def __init__(self, in_channels, mid_chnls=16, checkpointing=False, depth=7):
        # assert in_channels % 2 == 0
        out_c = in_channels // 2
        in_c = in_channels - out_c
        # print('> Using DN with SPP')
