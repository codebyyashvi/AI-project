import torch
from models.flow.denseflow.transforms.surjections import Surjection
...
class SimpleSortSurjection(Surjection):
    ...
    def forward(self, x):
        ...
        assert x_order.dim() == 2, 'lambd should return a tensor of shape (batch_size, dim_size) = ({}, {}), not {}'.format(
            x.shape[0], x.shape[self.dim], x_order.shape)
        assert x_order.shape[0] == x.shape[
            0], 'lambd should return a tensor of shape (batch_size, dim_size) = ({}, {}), not {}'.format(x.shape[0],
                                                                                                         x.shape[
                                                                                                             self.dim],
                                                                                                         x_order.shape)
        assert x_order.shape[1] == x.shape[
            self.dim], 'lambd should return a tensor of shape (batch_size, dim_size) = ({}, {}), not {}'.format(
            x.shape[0], x.shape[self.dim], x_order.shape)
        ...
        ldj = - self.buffer.new_ones(x.shape[0]) * torch.arange(1, 1 + x.shape[self.dim]).type(
            self.buffer.dtype).log().sum()
        return z, ldj
