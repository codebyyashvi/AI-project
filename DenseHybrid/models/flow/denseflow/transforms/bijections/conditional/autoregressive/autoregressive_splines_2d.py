import torch
from models.flow.denseflow.utils import sum_except_batch
from models.flow.denseflow.transforms.bijections.functional import splines
from models.flow.denseflow.transforms.bijections.conditional.autoregressive import ConditionalAutoregressiveBijection2d
...
class ConditionalLinearSplineAutoregressiveBijection2d(ConditionalAutoregressiveBijection2d):
    ...
    def __init__(self, autoregressive_net, num_bins, autoregressive_order='raster_cwh'):
        super(ConditionalLinearSplineAutoregressiveBijection2d, self).__init__(autoregressive_net=autoregressive_net,
...
class ConditionalQuadraticSplineAutoregressiveBijection2d(ConditionalAutoregressiveBijection2d):
    ...
    def __init__(self, autoregressive_net, num_bins, autoregressive_order='raster_cwh'):
        super(ConditionalQuadraticSplineAutoregressiveBijection2d, self).__init__(autoregressive_net=autoregressive_net,
                                                                                  autoregressive_order=autoregressive_order)
        self.num_bins = num_bins
    ...
    def _elementwise_forward(self, x, elementwise_params):
        assert elementwise_params.shape[-1] == self._output_dim_multiplier()
        unnormalized_widths, unnormalized_heights = elementwise_params[..., :self.num_bins], elementwise_params[...,
                                                                                             self.num_bins:]
        z, ldj_elementwise = splines.quadratic_spline(x, unnormalized_widths=unnormalized_widths,
                                                      unnormalized_heights=unnormalized_heights, inverse=False)
        ldj = sum_except_batch(ldj_elementwise)
        return z, ldj
    ...
    def _elementwise_inverse(self, z, elementwise_params):
        ...
        unnormalized_widths, unnormalized_heights = elementwise_params[..., :self.num_bins], elementwise_params[...,
                                                                                             self.num_bins:]
        x, _ = splines.quadratic_spline(z, unnormalized_widths=unnormalized_widths,
                                        unnormalized_heights=unnormalized_heights, inverse=True)
        return x
...
class ConditionalCubicSplineAutoregressiveBijection2d(ConditionalAutoregressiveBijection2d):
    ...
    def __init__(self, autoregressive_net, num_bins, autoregressive_order='raster_cwh'):
        super(ConditionalCubicSplineAutoregressiveBijection2d, self).__init__(autoregressive_net=autoregressive_net,
                                                                              autoregressive_order=autoregressive_order)
        self.num_bins = num_bins
...
class ConditionalRationalQuadraticSplineAutoregressiveBijection2d(ConditionalAutoregressiveBijection2d):
    ...
    def __init__(self, autoregressive_net, num_bins, autoregressive_order='raster_cwh'):
        super(ConditionalRationalQuadraticSplineAutoregressiveBijection2d, self).__init__(
            autoregressive_net=autoregressive_net, autoregressive_order=autoregressive_order)
        self.num_bins = num_bins
