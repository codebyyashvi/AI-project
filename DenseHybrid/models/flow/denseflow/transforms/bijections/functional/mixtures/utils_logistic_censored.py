import math
import torch
from models.flow.denseflow.transforms.bijections.functional.mixtures.utils_logistic import logistic_log_cdf, \
    logistic_log_one_minus_cdf, logistic_log_pdf
...
def cmol_cdf(x, means, log_scales, K):
    ...
    logistic_eval_lower = - (1 + 1 / (K - 1)) * torch.ones(torch.Size(), dtype=x.dtype, device=x.device)[
        (None,) * x.dim()]
    logistic_eval_upper = (1 + 1 / (K - 1)) * torch.ones(torch.Size(), dtype=x.dtype, device=x.device)[
        (None,) * x.dim()]
    cdf_mid = logistic_log_cdf(logistic_eval_x, means, log_scales).exp()
    cdf_lower = (1 - K * x) * logistic_log_cdf(logistic_eval_lower, means, log_scales).exp() * torch.lt(x, 1 / K).type(
        x.dtype)
    cdf_upper = (K * x - (K - 1)) * logistic_log_one_minus_cdf(logistic_eval_upper, means, log_scales).exp() * torch.gt(
        x, (K - 1) / K).type(x.dtype)
