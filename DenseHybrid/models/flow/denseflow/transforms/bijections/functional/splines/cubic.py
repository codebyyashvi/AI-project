...
from models.flow.denseflow.transforms.bijections.functional.splines.utils import searchsorted, cbrt
...
def cubic_spline(inputs,
    ...
    """
    assert left <= torch.min(inputs) and torch.max(inputs) <= right, 'Inputs < 0 or > 1. Min: {}, Max: {}'.format(
