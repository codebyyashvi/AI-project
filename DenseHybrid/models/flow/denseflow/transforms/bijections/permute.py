...
from models.flow.denseflow.transforms.bijections import Bijection
...
class Permute(Bijection):
    ...
    def __init__(self, permutation, dim=1):
        ...
        assert isinstance(permutation, torch.Tensor) or isinstance(permutation,
                                                                   Iterable), 'permutation must be a torch.Tensor or Iterable'
        if isinstance(permutation, torch.Tensor):
            assert permutation.ndimension() == 1, 'permutation must be a 1D tensor, but was of shape {}'.format(
                permutation.shape)
        else:
    ...
    def forward(self, x):
        return torch.index_select(x, self.dim, self.permutation), torch.zeros(x.shape[0], device=x.device,
                                                                              dtype=x.dtype)
