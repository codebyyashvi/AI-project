...
from models.flow.denseflow.transforms import Transform, Squeeze2d, Slice, WaveletSqueeze2d, SwitchChannels
from models.flow.denseflow.transforms import Conv1x1, BatchNormBijection2d, ActNormBijection2d
from models.flow.denseflow.distributions import StandardNormal
        ...
        for i in range(self.num_steps):
            ...
            layers = [
                SequentialTransform(nn.ModuleList([
                    ...
                    Conv1x1(chnls),
                    # SwitchChannels(),
                    SingleAffineCoupling(chnls, mid_chnls=layer_mid_chnls, checkpointing=checkpointing, depth=depth),
                    SwitchChannels()
...
if __name__ == '__main__':
    idb = InvertibleDenseBlockTransition(4, 1)
    x = torch.randn(2, 4, 6, 6)
    out, ld = idb(x)
    print(out.shape)
    x_hat = idb.inverse(out)
