...
def load_street_hazards_full(dataroot, bs, train_transforms, val_transforms, num_workers=3):
    train_set = StreetHazardsFull(dataroot, split='training',
                                  image_transform=None if not train_transforms['image'] else tf.Compose(
                                      train_transforms['image']),
                                  target_transform=None if not train_transforms['target'] else tf.Compose(
                                      train_transforms['target']),
                                  joint_transform=None if not train_transforms['joint'] else tf.Compose(
                                      train_transforms['joint']))
